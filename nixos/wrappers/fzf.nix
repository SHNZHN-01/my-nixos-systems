{ ... }: {
  flake.wrappers.fzf =
    ...
    }:
    {
      ...
      config.env.FZF_DEFAULT_COMMAND = lib.mkIf (
        config.fzf_default_command != null
      ) config.fzf_default_command;
      config.env.FZF_DEFAULT_OPTS = lib.mkIf (config.fzf_default_opts != null) config.fzf_default_opts;
      config.env.FZF_DEFAULT_OPTS_FILE = lib.mkIf (
        config.fzf_default_opts_file != null
      ) config.fzf_default_opts_file;
    };
}
